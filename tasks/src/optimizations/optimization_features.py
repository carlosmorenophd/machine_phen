...
class FeatureSelection:
    ...
    def _backward_feature_selection(
        ...
        list_models = [model.machine_name.value for
                       model in self._procedure.models]

        for model in self._procedure.models:
            print(f"Start with model {
                model.machine_name.value
            } from {list_models}")
    ...
    def _get_id_to_save(
        ...
        if machine.rebuild_hyper_parameters is not None:
            for rebuild_hyper_parameter in machine.rebuild_hyper_parameters:
                key_id = f"{
                    rebuild_hyper_parameter.name
                }_{machine.machine_name}"
    ...
    def _run_model_combination(
        ...
        Args:
            ...
            machine (MachineRegression): machine to run
        """
        for current_chromosome in\
                self._generate_combination_true_with_all_false(
                    features_chromosome=features_chromosome
                ):
    ...
    def _generate_combination_true_with_all_false(self, features_chromosome):
        """
        Generates all possible combinations with at least one True value in
        a boolean array, returning the complete array with the combinations
        of True converted to False, but without returning arrays with only
        False values.
