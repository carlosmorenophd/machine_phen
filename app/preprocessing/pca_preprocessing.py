...
from numpy import abs, ndarray, cumsum, sum, arange, where
...
class PCA_Preprocessing():
    ...
    def graph_projection(self):
        ...
        plt.subplot(1, 2, 1)
        plt.scatter(self.pca_x[:, 0], self.pca_x[:, 1], c=self.target, cmap='viridis', alpha=0.8)
        ...
        plt.bar(bar_positions - bar_width / 2, self.loadings_by_variable[:, 0], width=bar_width, label='PC1', alpha=0.8)
        plt.bar(bar_positions + bar_width / 2, self.loadings_by_variable[:, 1], width=bar_width, label='PC2', alpha=0.8)
        plt.xlabel('Variable')
        plt.ylabel('Carga en el Componente Principal')
        plt.title('Contribución de Variables en los Dos Primeros Componentes Principales')
    ...
    def graph_outlier(self, threshold = 2.5):
        ...
        for i in range(num_components):
            for j in range(i + 1, num_components):
                ...
                plt.scatter(self.pc_scores[:, i], self.pc_scores[:, j], edgecolors='k', c='none', marker='o', alpha=0.8)
                plt.scatter(self.pc_scores[outliers[0], i], self.pc_scores[outliers[0], j], edgecolors='r', facecolors='r', marker='o', s=200, label='Outliers')
