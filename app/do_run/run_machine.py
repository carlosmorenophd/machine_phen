...
class Result_ML:
    ...
    def write_to_csv(self):
        df = pd.DataFrame(
            self.results, columns=self.columns_name
        )
...
class Do_Run_ML:
    def __init__(
        ...
        self.files_dataset = {}
        self.machines = []
        self.results = Result_ML(columns_name=['file_key','file_name','ml','RMSE','R2','MAPE'], file_to_save=file_name_save)
        ...
        for key_file in self.files_dataset:
            for file in self.files_dataset[key_file]['datasets']:
                ...
                for machine in machine_to_run:
                    ...
                    self.results.adding_value(column_name='file_name', value=path.basename(file)) 
